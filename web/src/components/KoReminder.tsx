...
// Home-page reminder so nobody forgets the knockout tips. Shows whenever there's an
// open (drawn, not-yet-started) KO match — gold + a clear CTA while anything is
// untipped, a calm green "all set" once you're done. Opens the betting modal directly.
export function KoReminder() {
  ...
  const allDone = !!name && remaining === 0;

  return (
    <button className={`kor ${allDone ? "done" : "open"}`} onClick={() => setSheet(true)}>
      <span className="kor-ic">{allDone ? "✅" : "🏆"}</span>
      <span className="kor-txt">
        <b>Slutspelstips {!allDone && total > 0 ? "är öppet" : ""}</b>
        <span className="kor-sub">
          {!name
      ...
      {allDone ? <span className="kor-go">›</span> : <span className="kor-cta">{name ? "Tippa" : "Logga in"}</span>}
      <style>{`
