...
const SVT_URL = "https://www.svtplay.se/fifa-fotbolls-vm-2026";
const TV4_URL = "https://www.tv4play.se/kategorier/fifa-fotbolls-vm-2026";
...
// m.away are the uppercase TLAs). Unknown pairings (knockouts, gaps) return the
// generic SVT / TV4 fallback so there's always a sensible "where to watch".
export function broadcastForPair(homeTla?: string | null, awayTla?: string | null): BroadcastInfo {
  const key = homeTla && awayTla ? [homeTla.toUpperCase(), awayTla.toUpperCase()].sort().join("|") : "";
  const b = key ? BY_TLA_PAIR[key] : undefined;
  if (b === "svt") return { broadcaster: "svt", label: "SVT", channels: ["SVT1/SVT2", "SVT Play"], url: SVT_URL, free: true };
  if (b === "tv4") return { broadcaster: "tv4", label: "TV4", channels: ["TV4", "TV4 Play"], url: TV4_URL, free: false };
  return { broadcaster: null, label: "SVT / TV4", channels: ["SVT Play", "TV4 Play"], url: SVT_URL, free: false };
}
